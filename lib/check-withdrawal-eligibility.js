...
const getIssueClosedEvents = (issueId, oauthToken, after = null, result = { events: [], body: '' }) => {
  ...
  return axios
    ...
    .then(res => {
      ...
      if (hasNextPage) {
        return getIssueClosedEvents(issueId, oauthToken, res.node.timelineItems.pageInfo.endCursor, result);
      } else {
        return result;
      }
    }).catch(e => {
...
module.exports = async (username, issueId, oauthToken) => {
  return getIssueClosedEvents(issueId, oauthToken, null, null)
    .then(result => {
