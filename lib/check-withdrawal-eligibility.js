    ...
    await getIssueClosedEvents(issueId, oauthToken)
      .then(response => {
        ...
        if (data.errors && data.errors[0].type == "NOT_FOUND") {
          return reject({ canWithdraw: false, type: "NOT_FOUND", message: `No issue found with id ${issueId}.` });
        }
        ...
        if (node.timelineItems.nodes[0].closer == null) {
          return reject({ canWithdraw: false, type: "ISSUE_NOT_CLOSED_BY_PR", reason: `Issue was not closed by a PR` });
        }
        ...
        if (closer == viewer) {
          return resolve({ canWithdraw: true, type: "CAN_WITHDRAW", reason: `User ${viewer} closed issue ${issueId} with pull request ${prUrl}.` });
        } else {
          return reject({ canWithdraw: false, type: "ISSUE_NOT_CLOSED_BY_USER", reason: `Issue was not closed by ${viewer}. It was closed by ${closer}` });
        }
      }).catch(error => {
        ...
        reject({ canWithdraw: false, type: "UNKNOWN_ERROR", message: error.message });
      });
  });
};
