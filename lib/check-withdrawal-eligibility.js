...
const checkWithdrawalEligibility = async (issueUrl, oauthToken) => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (node.closed != true) {
        return reject({ issueId, canWithdraw: false, type: "NOT_CLOSED", message: `The issue with at ${issueUrl} is still open` });
      }
      ...
      if (closer == viewer) {
        ...
        return reject({ issueId, canWithdraw: false, type: "ISSUE_NOT_CLOSED_BY_USER", message: `Issue was not closed by ${viewer}. It was closed by ${closer}` });
      }
    } catch (error) {
      if (error.response && error.response.status == 401) {
